...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    const response = await fetch(
      `http://127.0.0.1:3000/api/v1/requests/getReq?requestId=${requestId}`,
      {
        method: 'GET',
        credentials: 'include',
      }
    ...
    document.getElementById('requested-by').textContent =
      request.requestinguser || 'N/A';
    document.getElementById('request-time').textContent =
      new Date(request.startTime).toLocaleString() || 'N/A';
  } catch (error) {
    console.error(error);
...
async function submitDemand() {
  ...
  try {
    ...
    const resp = await fetch(
      `http://127.0.0.1:3000/api/v1/technicians/sendConfirmation/${requestId}`,
      {
        method: 'GET',
        credentials: 'include',
      }
    ...
    if (resp.ok) {
      ...
      alert(`Error occurred: ${result.messsage}`);
    }
  } catch (error) {
    console.log(error);
  }
